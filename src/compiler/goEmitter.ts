...
namespace ts {
    ...
    function emitDeclarations(host: EmitHost, resolver: EmitResolver, emitterDiagnostics: DiagnosticCollection, declarationFilePath: string,
        ...
        function emitModuleElement(node: Node) {
            switch (node.kind) {
        ...
        function emitBlock(body: Block | Expression) {
            write("{");
            increaseIndent()
            if (isBlock(body)) {
        ...
        function emitIdentifier(node: Identifier) {
            let text = getTextOfNode(node, /*includeTrivia*/ false)
            if (!node.isSpecialName) {
                write(mangleName(text));
            } else {
                write(text);
            }
        }
